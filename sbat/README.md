...
This library handles [SBAT][SBAT.md] parsing and SBAT revocation checks.
...
component is not in the revocation list at all, so any generation is
allowed. The `sbat` component is in the revocation list, but since the
generation is 1, and 1 is the lowest possible generation number, all
versions of `sbat` are allowed.
