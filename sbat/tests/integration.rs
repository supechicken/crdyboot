...
//! Modifications:
//!
//! - GRUB_DEBIAN_BUG2_1 has been updated to bump the grub version from
//!   1 to 2, which seems to better match the example.
//!   text. https://github.com/rhboot/shim/pull/426

use sbat::{Allowed, MetadataReader, RevocationsReader, Revoked};
...
fn assert_revoked(revocations: &[u8], metadata: &[u8]) {
    let revocations = RevocationsReader::new(revocations).unwrap();
    let revocations = revocations.revocations().unwrap();
...
fn assert_allowed(revocations: &[u8], metadata: &[u8]) {
    let revocations = RevocationsReader::new(revocations).unwrap();
    let revocations = revocations.revocations().unwrap();
