...
impl<'a> Disk<'a> {
    ...
    pub fn info(&mut self) -> DiskInfo {
        DiskInfo {
            info: vb2_disk_info {
                ...
                bytes_per_lba: self.io.bytes_per_lba(),
    ...
    /// Convert `num_bytes` to number of blocks. Fail if the block size
    /// is invalid, or if the input is not an even multiple of the block
    /// size.
    fn bytes_to_blocks(&self, num_bytes: u64) -> Option<u64> {
        let bytes_per_lba = self.io.bytes_per_lba();

        if bytes_per_lba == 0 {
            error!("invalid bytes_per_lba: {}", bytes_per_lba);
            return None;
        }
    ...
    /// if the block size is invalid, or if overflow occurs.
    fn blocks_to_bytes_usize(&self, num_blocks: u64) -> Option<usize> {
        ...
        let num_bytes = bytes_per_lba.checked_mul(num_blocks)?;
