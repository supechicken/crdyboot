...
Crdyboot supports loading the flexor kernel when the `flexor` feature is
enabled. Crdyboot first attempts to load the ChromeOS kernel and if that fails,
it validates and loads the flexor kernel. It searches for a file named
`flexor_vmlinuz` in the system partitions and then validates it against a hard
coded list of SHA256 hashes (this is an interim solution, the hardcoded hashes
will eventually be replaced by more flexible signature checks ([b/361558595])).
This validated kernel data is then used for the
subsequent boot process.

The SHA256 hash is generated by running `sha256sum flexor_vmlinuz`. These hashes
are part of the chain of trust from our shim to executing code on user devices,
so reviewers must confirm the hash by downloading the file and verifying the
hash. All hashes must be for signed production builds. Never add a signature for
an unsigned dev/test image (however using the dev _channel_ is allowed).

[Flex Remote Deployment Release Process][FRD Release Process]
...
[FRD Release Process]: go/cros-frd-handover#heading=h.v0g5301i6zt5
[b/361558595]: b/361558595