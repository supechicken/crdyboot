...
impl<'a> Sbat<'a> {
    ...
    /// Write the Rust code that, when included in a binary, will add an
    /// `.sbat` section.
    ///
    /// The file is written to `<dir>/sbat_section.rs`, and can be
    /// included like this:
    ///
    /// ```no_compile
    /// include!(concat!(env!("OUT_DIR"), "/sbat_section.rs"));
    /// ```
    pub fn write_rust_file(&self, dir: &Path) {
    ...
    fn rust_link_section(&self) -> String {
        let csv = self.to_csv();
