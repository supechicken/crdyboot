...
section to a binary. Generating the metadata programatically helps to
avoid typos in the CSV, and also allows information to be pulled in from
other sources. For example, the package URL can be read from the
`CARGO_PKG_REPOSITORY` env var at build time.
