...
#[throws]
fn run_tests_for_package(package: Package, miri: bool) {
...
fn run_tests(action: &TestAction) {
    run_tests_for_package(Package::Xtask, false)?;
    run_tests_for_package(Package::Vboot, false)?;
    run_tests_for_package(Package::Libcrdy, false)?;

    if action.miri {
        run_tests_for_package(Package::Vboot, true)?;
        run_tests_for_package(Package::Libcrdy, true)?;
    }
}
