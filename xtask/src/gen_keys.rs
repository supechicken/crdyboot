...
fn gen_keypair(futility_path: &Utf8Path, key_paths: &VbootKeyPaths) {
    Command::with_args(
        ...
        &[
            "--vb1",
            "create",
            // Input key path, a private PEM key.
            key_paths.priv_pem().as_str(),
            // Base path for the output files. Two files are generated,
            // a .vbprivk file and a .vbpubk file. These extensions will
            // be appended to the provide base path.
